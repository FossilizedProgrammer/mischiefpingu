...
/// ═══════════════════════════════════════════════════════════════
///  WireGuardAdapter — اتصال WireGuardHealthSource به
///  TunnelHealthMonitor.
///
///  ⚠️ تغییر: حالا از `WireGuardHealthSource` استفاده می‌کند
///  تا منطق شمارش موفقیت/شکست و emit report متمرکز باشد.
/// ═══════════════════════════════════════════════════════════════
class WireGuardAdapter {
  final void Function(String message, {String source}) log;
  final TunnelHealthMonitor monitor;

  late final WireGuardHealthSource _source;

  WireGuardAdapter({required this.log, required this.monitor}) {
    _source = WireGuardHealthSource(
      log: log,
      onReport: ({
